 ...
 * 
 * Production MCP server for browser automation using Browserbase and Stagehand.
 * SessionManager maintains state automatically on persistent platforms like Render.
 */
...
const handler = createMcpHandler(
  async (server) => {
    ...
    // Use a single context ID for the entire process (persists across requests)
    const contextId = 'mcp-context';
    ...
    for (const tool of TOOLS) {
      ...
      server.tool(
        ...
        async (params: any) => {
          try {
            // SessionManager maintains state automatically
            // If sessionId is provided, just set it as active
